---
title: "QTM 151"
subtitle: "Week 3 -- dplyr (join datasets) <html><div style='float:left'></div><hr color='#f2a900' size=1px width=800px></html>"
...
`dplyr` is excellent for the following data manipulation: 
+ filter
+ arrange
+ select
+ rename
...
* **dplyr is also great to merge datasets**
]
...
* A key is a variable that uniquely identifies an observation. Otherwise, we need multiple variables to identify the observation.
...
A `full_join` keeps observations that appear in either x or y.
...
*anti_join()* also provides a great way to diagnose joins that go wrong.
