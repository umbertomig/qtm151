...
We've learned:
 ...
 + `plotly` methods: create beautiful interactive plots.
 
**Great job!!** Do you have any questions about these contents?

Reminder: **no class next Friday**. I'll be at a conference.

Today we will learn how to make **maps in R**. Our GitHub page is: https://github.com/umbertomig/qtm151
...
Let's plot the number of flights from each destination.
...
* `group`: ggplot2’s functions can take a group argument, which controls whether lines should connect adjacent points. 
  + If they are in the same group, they get connected, but if they are in different groups, they don't.
...
*geom_polygon()* draws lines between points and “closes them up” (i.e. draws a line from the last point back to the first point). 
...
* coord_fixed(): it fixes the relationship between one unit in the y-direction and one in the x-direction. 
...
This is helpful to plot counties in Georgia:
